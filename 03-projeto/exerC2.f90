program exerC2 ! método de euler-cromer; pêndulo (aproximação harmônica)
      implicit none

        ! declaração das variáveis
        real(8), parameter :: g = 10.0d0, pi = (4 * atan(1.0_8))
        real(8) :: l, m, T, deltaT
        real(8) :: theta, theta_zero, w, w_zero = 0.d0, E
        ...
        read *, deltaT
        read *, T
        open(1, file = "exerC2_out.dat", status = "replace")
        open(2, file = "energia2.dat", status = "replace")
        ...
        do i = 1, n
                ...
                theta = (theta_zero + (w * deltaT)) 
                ! a diferença em relação ao método de euler está na expressão
                ! acima, na qual w_zero (anterior) dá lugar a w (posterior)
